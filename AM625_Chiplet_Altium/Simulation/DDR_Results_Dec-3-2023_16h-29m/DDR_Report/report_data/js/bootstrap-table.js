...
!function ($) {
                ...
                for(k = jCurrentCol; k < (jCurrentCol + jColSpan); k++)
                {
                    ...
                    if(i > 0) {
                        ...
                        $.each(vColumns[k].slice(0,i), function(vcolInd,vcolVal) {
                            if (columns[i][j].parents.indexOf(vcolVal) == -1) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        // Patch: calculate the colspans of parents in the right fashion
        $.each(this.options.columns, function(i, columns) {
            $.each(columns, function(j, column) {
                column.colspan = 0;
            })
        });
        $.each(this.options.columns[this.options.columns.length - 1], function(i, column) {
            column.colspan = 1;
            ...
            if (typeof column.parents !== 'undefined') {
                $.each(column.parents,function(k,lookupStr) {
                    var lookupKeys = lookupStr.split("."),
                        ...
                        parentCol.colspan += (column.visible) ? 1 : 0;
