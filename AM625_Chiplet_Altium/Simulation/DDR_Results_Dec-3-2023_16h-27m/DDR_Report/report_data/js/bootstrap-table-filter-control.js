...
!function ($) {
    ...
    var addOptionToSelectControl = function (selectControl, value, text) {
        ...
        // Patch: remove HTML from value
        value = Pyr.stripHTML(value)
        ...
        $.each(that.columns, function (i, column) {
            ...
            if (!column.filterControl) {
                ...
                switch (column.filterControl.toLowerCase()) {
                    case 'input' :
                        html.push(sprintf('<input type="text" class="form-control" style="width: 100%; visibility: %s" placeholder="Filter">', isVisible));
                        break;
                    case 'select':
    ...
    BootstrapTable.prototype.initSearch = function () {
        ...
        if ("searchText" in that) {
            numsearch = (that.searchText.charAt(0) == "<" || that.searchText.charAt(0) == ">");
        }
        ...
        this.data = fp ? $.grep(this.data, function (item, i) {
            for (var key in fp) {
                ...
                if (numsearch) {
                    ...
                    var searchVal = fval.substring(1);
                    var searchVal = parseFloat(searchVal);
                    if (fval.charAt(0) == "<") {
                        if (!(numVal < searchVal)) return false
                    } else if (fval.charAt(0) == ">") {
                        if (!(numVal > searchVal)) return false
                    }
                } else {
    ...
    BootstrapTable.prototype.onColumnSearch = function (event) {
        ...
        if (text && this.options.height) { // Patch: fix for the fixed header bug which empties the select. Watch the " " at the end: that is the way BSTable stores the options :S
